...
**完整模式**：
   ...
   - 上限：3 轮
   - 收敛条件：连续两轮无新遗漏
   - 每轮检查：是否有遗漏的次要人物、地点、事件、职位角色等

**补充模式**：
   - 读取 raw-registry.md 的"剩余遗漏"字段
   - 针对遗漏实体提取详情
   - 执行迭代检查新遗漏（包括次要人物、地点、事件、职位角色）
...
3. **细节敏感**：职位角色、产品项目等细节内容也应提取
4. **模板参考**：templates 用于约束 wiki 页面格式，不约束提取范围
...
| 检查项 | 示例 |
|--------|------|
| 次要人物 | 文中提及但未提取的人物（如配角、协作者） |
| 地点信息 | 文中提及的地点（如办公室、城市） |
| 补充事件 | 文中提及的事件（如会议、活动） |
| 职位角色 | 人物的职位、角色（如经理、工程师） |
| 产品项目 | 文中提及的产品、项目名称 |
| 时间节点 | 文中提及的时间、阶段（如2024年Q1） |
